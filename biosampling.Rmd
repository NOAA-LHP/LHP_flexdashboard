...
source(here::here("functions", "dataplot_simple.R")) #put into the rmardown instead
...
#data <- read.csv("data/all_samples_2_2024.csv")
data <- read.csv("data/all_samples_2_2024_commonname.csv")
...
div(style = "margin-top: -20px; ", #decreases amount of whitespace between label and input selection UI
  selectInput('Name', label = ' ', choices = common_list, selected = c('SILVERMOUTH'))
)
...
output$text_summary <- renderUI({
  ...
  # Filter data
  temp <- data |> filter(Region == input$Region, ScientificName == input$Species | CommonName==input$Name)
  #for checking code 
  #temp<-data
  #temp<-temp%>% filter(ScientificName=="Aphareus rutilans" & Region=="Guam")
...
Pacific Islands Fisheries Science Center, Life History Program samples insular fish species in the U.S. Pacific jurisdictions via the Commercial Fisheries Biosampling Programs (CFBS) and NOAA Life History Program research surveys. This dashboard summaries our biosampling collections to date. For more information please see the full biosamling inventory report available at: [https://repository.library.noaa.gov/view/noaa/45018](https://repository.library.noaa.gov/view/noaa/45018).
